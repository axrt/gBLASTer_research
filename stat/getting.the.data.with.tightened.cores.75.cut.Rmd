---
title: "Attempt to tighten the cores by shifting the cutoff to 0.75"
output: html_document
---

See [previous attmept](getting.the.data.separated.by.core.counts.wide.margin.Rmd), where we tried to throw away the margin between the cores and outliers. Didn't really work out because fo the following porblem:

Suppose, gene A had `core, outsider, outsider, nosep`. However, both outsiders in the center went into the margin and hence А had `core, nosep, nosep, nosep`, and thereby it is condsidered a core gene, when it is wildly not.
This time I skiped the justification for the minimal Rsquare = 0.25, see the document by the link above for the plot.
...
This time we are processing the data with no margin, but with a strict cutoff of 0.75 on the genes that may go into the cores.

```{r read models}
dir<-"data" #data folder
factor.core<-c("core","outsider","nosep") #generate three factors to mark states
if(!file.exists("master.table.core.0.75.cut.txt")){
...
Getting the final results.. btw, i relly need to make a function of the above

```{r get the final results}

#read the data
master.table.core.0.75.cut<-read.table(file="master.table.core.0.75.cut.txt")
...
master.table.core.0.75.cut.per.gene.summary<-as.data.frame(t(parApply(cl=cluster, X = master.table.core.0.75.cut, MARGIN = 1, FUN = function(i){
...
master.table.core.0.75.cut.per.gene.summary$gene.name<-row.names(master.table.core.0.75.cut.per.gene.summary)
#do not forget to cluse the cluster
stopCluster(cluster)
#separate the "always core table"
master.table.core.0.75.cut.always.core<- master.table.core.0.75.cut.per.gene.summary %>% filter(outsider==0)
head(master.table.core.0.75.cut.always.core)
#separate the "outsider once"
master.table.core.0.75.cut.outsider.once<- master.table.core.0.75.cut.per.gene.summary %>% filter(outsider==1)
head(master.table.core.0.75.cut.outsider.once)
#separate the frequent outsider
master.table.core.0.75.cut.outsider.frequently<- master.table.core.0.75.cut.per.gene.summary %>% filter(outsider>1)
head(master.table.core.0.75.cut.outsider.frequently)
#write out the data
write.table(master.table.core.0.75.cut.always.core, 
            file="master.table.core.0.75.cut.always.core.txt", sep="\t", col.names=TRUE, quote = FALSE)
write.table(master.table.core.0.75.cut.outsider.once, 
            file="master.table.core.0.75.cut.outsider.once.txt", sep="\t", col.names=TRUE, quote = FALSE)
write.table(master.table.core.0.75.cut.outsider.frequently, 
            file="master.table.core.0.75.cut.outsider.frequently.txt", sep="\t", col.names=TRUE, quote = FALSE)
#
```

Now into building trees

```{r loading data}
#first of all - legend
legend<-load.legend(legend.file = "/home/alext/Developer/gbra/inst/extradata/legend.csv",sep = "\t")
#this table contains all the names in the "gene.name" column
master.table.core.always.core<-read.table(file="master.table.core.0.75.cut.always.core.txt", header = TRUE, sep = "\t")
...
master.table.core.outsider.frequently<-read.table(file="master.table.core.0.75.cut.outsider.frequently.txt", header=TRUE, sep="\t")
#load the facultative outsiders
master.table.core.outsider.once<-read.table(file="master.table.core.0.75.cut.outsider.once.txt", header=TRUE, sep="\t")
#outsider table
